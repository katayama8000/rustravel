...
fn main() {
    ...
    let team_name: &str = "Bob";
    let new_score: i32 = 20;

    // エントリーが存在しない場合は新しいエントリーを作成
    let team_score: &mut i32 = scores.entry(team_name).or_insert(0);
    // 新しい値を設定
    *team_score += new_score;
    println!("{:#?}", scores);
    let team_score2: &mut i32 = scores.entry("Alice").or_insert(100);
